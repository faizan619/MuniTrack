...
export default function InComp({ users }) {
  ...
  return (
    <div className="text-white px-5 py-3">
          {users === undefined ? (
            <p className={`${arima.className}`}>Loading Data..</p>
          ) : users.length === 0 ? (
            <p className="text-white">No Data Found</p>
          ) : (
            <div className={`flex justify-start flex-wrap ${arima.className}`}>
              <div>
                <p className={`${serif.className}`}>Incharge Available : [ {users.length} ]</p>
                <div className="flex flex-wrap gap-3">
                  {users.map((item) => (
                    <div
                      key={item._id}
                      className="flex flex-wrap mr-5 gap-5 my-3"
                    >
                      <div className={`flex items-center gap-3 p-3 rounded-md ${user.email===item.email?"bg-violet-600 text-white":"bg-white text-black"} select-none`}>
                        <img
